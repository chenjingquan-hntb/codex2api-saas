...
func (h *Handler) RegisterRoutes(r *gin.Engine) {
	...
	// P6 收尾：管理员支付订单管理 / 冲正 / 对账 / 重算。
	api.GET("/payments/orders", h.ListAdminPayments)
	api.GET("/payments/orders/:order_no", h.GetAdminPayment)
	api.POST("/payments/orders/:order_no/refund", h.RefundAdminPayment)
	api.POST("/payments/orders/:order_no/confirm", h.ConfirmAdminPayment)
	api.POST("/payments/users/:id/adjust", h.AdjustAdminUserBalance)
	api.GET("/payments/summary", h.GetPaymentSummaryHandler)
	api.POST("/payments/reconcile", h.ReconcileAdminPayments)
	api.POST("/redeem/batches", h.CreateRedeemBatch)
	api.GET("/redeem/batches", h.ListRedeemBatches)
	api.GET("/redeem/batches/:id", h.GetRedeemBatch)
	api.GET("/redeem/batches/:id/codes", h.ListRedeemCodes)
	api.POST("/redeem/batches/:id/revoke", h.RevokeRedeemBatch)
